...
const Card = ({ event, listName, orgEvent }) => {
  ...
  const [price, setPrice] = useState(
    firstPublicDate !== undefined ? firstPublicDate.price : ""
  );
      ...
      <div className={styles.cardText}>
        {orgEvent === "true" && selectedDate === "" ? (
          <p className={styles.cardDateCurrent}>
            {event.dates[0].dateFormated.replace("de", "/")}
          </p>
        ) : orgEvent === "true" && selectedDate !== "" ? (
          <p className={styles.cardDateCurrent}>
            {selectedDate.replace("de", "/")}
          </p>
        ) : orgEvent !== "true" ? (
          <div>
              ...
              >
                {event.dates.map((date, index) =>
                  ...
                  date.inRevision === false ? (
                    date.dateFormated.slice(date.dateFormated.length - 4) ===
                    numCadena ? (
                      <option key={index} value={date.price}>
                        {date.dateFormated.slice(
                          0,
                          date.dateFormated.length - 7
                        )}
                      </option>
                    ) : (
                      <option key={index} value={date.price}>
            ...
            ) : event.dates[0].dateFormated.slice(
                event.dates[0].dateFormated.length - 4
              ) === numCadena &&
              event.dates[0].isPublic === true &&
              event.dates[0].inRevision === false ? (
              <p className={styles.cardDateCurrent}>
                {event.dates[0].dateFormated.slice(
                  0,
                  event.dates[0].dateFormated.length - 7
                )}
              </p>
            ) : event.dates[0].isPublic === true &&
              event.dates[0].inRevision === false ? (
            ...
            {isLoading ? (
              ...
              <div>
                {event.organizer._id === user.uid || orgEvent === "true" ? (
                  ...
                  <div
                    className={styles.cardAddFavHeart}
      ...
      {organizer.length === 1 ? (
          ...
          <div>
            <div className={styles.cardOrgInfo}>
              ...
              <Link
                ...
                to={`/detalles-del-evento/${event._id}`}
              >
        ...
        <div>
          <div className={styles.cardOrgInfo}>
            <Link
              className={styles.link}
              to={`/sobre-el-organizador/${event.organizer._id}`}
            >
              <img
                className={styles.cardOrgPicture}
        ...
        <div className={styles.containerDatos}>
          <div className={styles.datos}>
            ...
            <div className={styles.subDatos}>
              ...
              <Link
                to={`/usuario/asistentes-al-evento/${event._id}/${selectedDateId}`}
              >
                <button>Ver</button>
              </Link>
            </div>
