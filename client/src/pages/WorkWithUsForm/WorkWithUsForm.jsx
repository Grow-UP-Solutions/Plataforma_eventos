    ...
    <div className={`${styles.pageWorkWithUsForm} container`}>
      ...
      <form onSubmit={(e) => e.preventDefault()}>
        ...
        <div className={styles.formGroup}>
          ...
          <input onChange={handleChangeInput} value={formData.name} name='name' type='text' id='name' required />
        </div>
        <div className={styles.formGroup}>
          <label htmlFor='numId'>Número de Identificación:</label>
          <input onChange={handleChangeInput} value={formData.numId} name='numId' type='text' id='numId' required />
        </div>
        <div className={styles.formGroup}>
          <label htmlFor='mail'>Email:</label>
          <input onChange={handleChangeInput} value={formData.mail} name='mail' type='text' id='mail' required />
        </div>
        <div className={styles.formGroup}>
          <label htmlFor='city'>Ciudad:</label>
          <input onChange={handleChangeInput} value={formData.city} name='city' type='text' id='city' required />
        </div>
        <div className={styles.formGroup}>
          <label htmlFor='tel'>Teléfono:</label>
          <input onChange={handleChangeInput} value={formData.tel} name='tel' type='text' id='tel' />
        </div>
        <div className={styles.formGroup}>
          <label htmlFor='phone'>Celular:</label>
          <input onChange={handleChangeInput} value={formData.phone} name='phone' type='text' id='phone' required />
        </div>
        ...
        <div className={styles.containerCV}>
          ...
          <div className={styles.dragCv}>
            ...
            {fileCV.type === 'application/pdf' ? (
              ...
              fileCV.type === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' ? (
            ...
            <span
              style={{
                color: fileCV.status === 'success' ? 'green' : fileCV.status === 'failed' ? 'red' : '',
              }}
            >
              {fileCV.message}
            </span>
        ...
        <div className={styles.overlay}>
          <div className={styles.containerOverlay}>
            ...
            <div className={styles.overlayInfo}>
              <h2 className={styles.overlayTitle}>Haz aplicado a nuestra vacante de {work.replace(':', '')}</h2>
              <p>Tu solicitud ha sido enviada con éxito. Pronto recibirás una respuesta a tu correo.</p>
              <button onClick={() => setShowOverlay(false)} className={styles.btnOverlay}>
                Listo
              </button>
            </div>
          </div>
